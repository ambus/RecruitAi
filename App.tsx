...
import { Candidate, InterviewSession, Score, Question } from './types';
import { QUESTIONS as INITIAL_QUESTIONS, DEFAULT_CATEGORIES } from './constants';
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-slate-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl">
          ...
          <button onClick={() => setView('HISTORY')} className="group p-8 bg-white border-2 border-transparent hover:border-blue-500 rounded-3xl shadow-xl transition-all flex flex-col items-center gap-4 text-center">
            <div className="w-16 h-16 bg-slate-100 text-slate-600 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform"><svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
            <h3 className="text-xl font-bold text-slate-800">Historia Rozmów</h3>
          </button>
          <button onClick={() => setView('QUESTIONS_MGMT')} className="group p-8 bg-white border-2 border-transparent hover:border-blue-500 rounded-3xl shadow-xl transition-all flex flex-col items-center gap-4 text-center">
            <div className="w-16 h-16 bg-purple-100 text-purple-600 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform"><svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
      ...
      <div className="min-h-screen p-6 md:p-12 max-w-5xl mx-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={() => setShowCategoryMgmt(true)} className="px-4 py-2 bg-purple-600 text-white font-bold rounded-lg hover:bg-purple-700 transition-colors shadow-sm">Kategorie</button>
            <button onClick={handleExportQuestions} className="px-4 py-2 bg-white border border-slate-200 text-slate-600 font-bold rounded-lg hover:bg-slate-50 transition-colors flex items-center gap-2 shadow-sm"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>Eksport</button>
            <button onClick={() => questionsFileInputRef.current?.click()} className="px-4 py-2 bg-white border border-slate-200 text-slate-600 font-bold rounded-lg hover:bg-slate-50 transition-colors flex items-center gap-2 shadow-sm"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>Import</button>
            <input type="file" ref={questionsFileInputRef} onChange={handleImportQuestionsFile} accept=".json" className="hidden" />
            <button onClick={() => setEditingQuestion({ category: categories[0] || '' })} className="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors shadow-md">+ Pytanie</button>
                ...
                {categories.map(cat => (
                  <div key={cat} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => setEditingCategory({old: cat, new: cat})} className="p-1 text-slate-400 hover:text-blue-600"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
                      <button onClick={() => handleDeleteCategory(cat)} className="p-1 text-slate-400 hover:text-red-500"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                    </div>
                  </div>
                ))}
              </div>
             ...
             <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg">
                ...
                <form onSubmit={handleSaveQuestion} className="space-y-4">
                   ...
                   <div>
                      ...
                      <textarea className="w-full px-4 py-2 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 min-h-[100px] bg-white text-slate-900" value={editingQuestion.question || ''} placeholder="Treść..." required onChange={(e) => setEditingQuestion({...editingQuestion, question: e.target.value})} />
                   </div>
                   <div>
        ...
        <div className="space-y-8">
           {questionsByCategory.map(catGroup => (
             <div key={catGroup.name}>
                <h2 className="text-xl font-black text-slate-800 border-l-4 border-purple-500 pl-4 mb-4">{catGroup.name}</h2>
                <div className="grid gap-3">
                   {catGroup.questions.map(q => (
                     <div key={q.id} className="bg-white p-5 rounded-xl border border-slate-200 flex justify-between items-center group">
                        <div className="flex-1 pr-8"><p className="font-semibold text-slate-700">{q.question}</p></div>
                        <div className="flex gap-2">
                           <button onClick={() => setEditingQuestion(q)} className="p-2 text-slate-400 hover:text-blue-600 transition-colors"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                           <button onClick={() => handleDeleteQuestion(q.id)} className="p-2 text-slate-400 hover:text-red-500 transition-colors"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                        </div>
                     </div>
                   ))}
  ...
  if (view === 'FORM') {
    return (
      <div className="min-h-screen py-12 px-4">
        <button onClick={() => setView('START')} className="mb-8 flex items-center gap-2 text-slate-500 hover:text-blue-600 transition-colors mx-auto max-w-md w-full"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>Menu</button>
    ...
    return (
      <div className="min-h-screen p-6 md:p-12 max-w-4xl mx-auto">
        ...
        <div className="grid gap-4">
          {history.map((h, idx) => (
            <div key={idx} onClick={() => { setSession(h); setView('SUMMARY'); }} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-blue-400 cursor-pointer transition-all flex items-center justify-between group">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center font-bold">{h.candidate.name.charAt(0)}</div>
                <div><h3 className="font-bold text-slate-800">{h.candidate.name}</h3><p className="text-slate-400 text-sm">{h.candidate.interviewDate}</p></div>
              </div>
    ...
    return (
      <div className="min-h-screen flex flex-col bg-slate-50">
        <nav className="bg-white border-b border-slate-200 sticky top-0 z-10 px-6 py-4 flex justify-between items-center shadow-sm">
          <div className="flex items-center gap-4">
            <button onClick={() => confirm('Anulować?') && setView('START')} className="p-2 text-slate-400 hover:text-red-500"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
            <h1 className="text-xl font-bold text-slate-800">{session.candidate.name}</h1>
          </div>
          <button onClick={handleFinish} disabled={isSummarizing} className="px-6 py-2 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700 shadow-md disabled:bg-slate-200">{isSummarizing ? 'Generowanie...' : 'Zakończ'}</button>
        </nav>
            ...
            <section key={category.name} className="space-y-4">
              ...
              <div className="grid gap-4">
                {category.questions.map((q) => {
                  ...
                  return (
                    <div key={q.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                      <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="flex-1 pr-4"><h3 className="text-lg font-semibold text-slate-700">{q.question}</h3></div>
                        <div className="flex items-center gap-4 shrink-0">
                          <RatingScale value={score?.rating || 0} onChange={(val) => handleRate(q.id, val)} />
                          <button onClick={() => setExpandedQuestion(isExpanded ? null : q.id)} className={`p-2 rounded-lg ${isExpanded ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'}`}><svg className={`w-5 h-5 transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg></button>
                        </div>
                      </div>
                      {isExpanded && <div className="px-5 py-4 bg-slate-50 border-t border-slate-100"><p className="text-slate-600 italic"><span className="text-xs font-black text-blue-600 uppercase block mb-1">Prawidłowa odpowiedź:</span>{q.correctAnswer}</p></div>}
                    </div>
                  );
                })}
              </div>
      ...
      <div className="min-h-screen p-6 md:p-12 max-w-4xl mx-auto">
        <div className="bg-white rounded-2xl shadow-2xl p-8 border border-slate-100">
          <header className="flex justify-between items-start mb-10 border-b pb-6">
            <div>
              <button onClick={() => setView('START')} className="mb-4 text-slate-400 hover:text-blue-600 flex items-center gap-1 text-sm font-bold">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>Wyjdź
              </button>
          ...
          <button onClick={() => setView('START')} className="w-full py-4 bg-slate-800 text-white font-bold rounded-xl hover:bg-slate-900 shadow-lg transition-all">Zamknij i Wróć</button>
        </div>
      </div>
    );
  }
